...
export function useOemLogosState(
  userId: string | undefined,
  defaultData: OemType[]
) {
  const [oemLogos, setOemLogos] = useState<OemType[]>(defaultData);
  ...
  const saveOemData = useCallback(async () => {
    if (!userId) return;
  ...
  }, [oemLogos, userId, loading, saveOemData]);
